...
const initialCode = `function helloGuys() {
    console.log("Welcome Live Coding Collaboration");
  }
  ...
  export const findParentDirectory = (
    ...
    //Checking the current directory mathces the parentName
    if(directory.id === parentDirId && directory.type === "directory"){
        return directory;
    }

    //Recursively searching children if it's directory
  ...
  export const sortFileSystemItem = (item:FileSystemItem): FileSystemItem =>{
    //Recursively sort children of it's a directory
